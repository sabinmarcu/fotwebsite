...
try
	...
	extractClass = (text, next) ->
		try
			cls = text.match /^(\.[a-zA-Z0-9]*)+/ 
			if cls? then next (text.replace cls[0], ""), (cls[0].split(".").join(" "))
			else next text, null
		catch e
			console.log e.message
	renderElement = (text, cls, element) ->
		if cls? then "<#{element} class='#{cls}'>#{text}</#{element}>"
		else "<#{element}>#{text}</#{element}>"
	wrapAddClass = (text, element) -> extractClass text, (text, cls) -> renderElement text, cls, element

	renderer.heading = (text, level) -> wrapAddClass text, "h#{level}"
	renderer.paragraph = (text) -> wrapAddClass text, "p"

	compilers.md = compilers.markdown = (module, filename) ->
		content = md (fs.readFileSync filename, "utf8"), renderer: renderer
		content = content.replace /\\?"/g, "\\\""
		content = content.replace /\r?\n/g, "\\n"
		module._compile "module.exports = \"#{content}\"", filename
catch err
...
try
	...
	if Handlebars and Handlebars.precompile
		compilers.hbs = compilers.handlebars = (module, filename) ->
...
try
	...
	compilers.jade = (module, filename) ->
		content = jade.compileClient ( fs.readFileSync filename, 'utf8' ), client:true
...
try
	mime = require "mime"
	compilers.eot = compilers.woff = compilers.ttf = compilers.otf = compilers.png = compilers.gif = compilers.jpg = compilers.svg = compilers.jpeg = (module, filename) ->
...
exports.Package = class Package
	...
	sharedStart: (arr...) ->
		...
		L = word1.length
		i = 0
		while i < L and word1.charAt(i) is word2.charAt(i)
			i++
		word1.substring 0, i
		...
		fs.realpath path, (err, sourcePath) =>
			...
			async.map @paths, fs.realpath, (err, expandedPaths) =>
				...
				for expandedPath in expandedPaths
					...
					if sourcePath.indexOf(base) isnt 0 then sourcePath = @sharedStart(base, sourcePath) + path
