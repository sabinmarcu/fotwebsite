...
class IndexedDBDriver extends IS.Object
    ...
    remove: (key, callback) ~>
        ...
        req.onsuccess = ~> @log "Successfuly deleted #key"; callback? null
        req.onerror = ~>  callback? it; throw new Error "Could not delete item #key"
        @
    open-database: !~>
        @log "Opening Database #{DB_NAME}, version #{DB_VERSION}"
    ...
    error-event: ~> @log "Database Error : ", it; console.log arguments
    upgrade-event: ~>
        db = it.target.result
